...
module Genova
  module App
    describe Client do
      let(:app_client) { Genova::App::Client.new('account', 'repository') }
      ...
      describe 'clone' do
        it 'should be execute git clone' do
          ...
          expect { app_client.clone }.to_not raise_error
        end
      end
      ...
      describe 'pull' do
        ...
        it 'should be get latest source' do
          allow(app_client).to receive(:clone)
          ...
          allow(app_client).to receive(:client).and_return(git_mock)

          expect { app_client.pull }.to_not raise_error
        end
      end

      describe 'load_deploy_config' do
        it 'should be return config' do
          allow(app_client).to receive(:pull)
          ...
          expect(app_client.load_deploy_config).to be_a(deploy_config_mock.class)
        end
      end

      describe 'task_definition_config_path' do
        it 'should be return task definition path' do
          expect(app_client.task_definition_config_path('./config/deploy/path.yml')).to eq(app_client.base_path + '/config/deploy/path.yml')
        end
      end

      describe 'load_task_definition_config' do
        it 'should be return config' do
          allow(app_client).to receive(:task_definition_config_path)
          allow(File).to receive(:exist?).and_return(true)
          allow(File).to receive(:read).and_return('{}')

          expect(app_client.load_task_definition_config('path')).to be_a(Genova::Config::TaskDefinitionConfig)
        end
      end
      ...
      describe 'origin_branches' do
        ...
        it 'should be return origin branches' do
          ...
          expect(app_client.origin_branches.size).to eq(1)
        end
      end
      ...
      describe 'find_commit_id' do
        ...
        it 'should be return commit id' do
          ...
          expect(app_client.find_commit_id('tag')).to eq('id')
        end
      end
    end
  end
