    ...
    class Util
      class << self
        ...
        def branch_options(account, repository, branch_limit = Settings.slack.interactive.branch_limit)
          app_client = Genova::App::Client.new(account, repository)
          branches = []
          size = 0

          app_client.origin_branches.each do |branch|
        ...
        def cluster_options(account, repository, branch)
          clusters = []
          app_client = Genova::App::Client.new(account, repository, branch)
        ...
        def target_options(account, repository, branch, cluster)
          ...
          app_client = Genova::App::Client.new(account, repository, branch)
          cluster_config = app_client.load_deploy_config.cluster(cluster)
